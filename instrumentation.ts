 ...
 *
 * Disabled by default because running a Prisma transaction at dev startup made
 * Next 16 + Neon feel slow and sometimes timed out. Run with:
 *   NEXT_PUBLIC_AUTO_INIT_DB=true pnpm dev
 * or use prisma migrate/seed manually when deploying a fresh database.
 */
export async function register() {
  if (process.env.NEXT_RUNTIME === 'edge') return
  if (process.env.NEXT_PUBLIC_AUTO_INIT_DB !== 'true' && process.env.AUTO_INIT_DB !== 'true') return

  try {
    const [{ db }, cuidModule] = await Promise.all([
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.configs.createMany({
        data: [
          ...
          { config_key: 'bucket', config_value: '', detail: '阿里 OSS / AWS S3 Bucket 存储桶名称，如：yuncan' },
          { config_key: 'storage_folder', config_value: '', detail: '存储文件夹(S3)，严格格式，如：yuncan 或 yuncan/images ，填 / 或者不填表示根路径' },
          ...
          { config_key: 'alist_token', config_value: '', detail: 'AList / OpenList 令牌' },
          { config_key: 'alist_url', config_value: '', detail: 'AList / OpenList 地址，如：https://disk.yuncan.com' },
          ...
          { config_key: 'r2_bucket', config_value: '', detail: 'Cloudflare Bucket 存储桶名称，如：yuncan' },
          { config_key: 'r2_storage_folder', config_value: '', detail: '存储文件夹(Cloudflare R2)，严格格式，如：yuncan 或 yuncan/images ，填 / 或者不填表示根路径' },
          { config_key: 'r2_public_domain', config_value: '', detail: 'Cloudflare R2 自定义域（公开访问）' },
          { config_key: 'cos_secret_id', config_value: '', detail: '腾讯云 COS SecretId' },
          { config_key: 'cos_secret_key', config_value: '', detail: '腾讯云 COS SecretKey' },
          { config_key: 'cos_region', config_value: '', detail: '腾讯云 COS 地域，如：ap-hongkong' },
          { config_key: 'cos_bucket', config_value: '', detail: '腾讯云 COS 存储桶名称，如：yuncan-123456789' },
          { config_key: 'cos_storage_folder', config_value: '', detail: '存储文件夹(腾讯云 COS)，严格格式，如：yuncan 或 yuncan/images ，填 / 或者不填表示根路径' },
          { config_key: 'cos_domain', config_value: '', detail: '腾讯云 COS 自定义域名（如有）' },
          { config_key: 'custom_title', config_value: 'yuncan', detail: '网站标题' },
    ...
    console.error('Optional database bootstrap failed.', e)
  }
}
